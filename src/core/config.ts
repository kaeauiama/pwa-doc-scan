/**
 * 閾値・定数の単一ソース。
 * ドキュメントには数値を直書きせず、この名前で参照すること(CLAUDE.md 参照)。
 */

export const CONFIG = {
  binarize: {
    /** 既定の手法。U-01 として未確定。実書類で比較して決める */
    method: "sauvola" as "sauvola" | "bradley",
    /** 局所窓のサイズ = 長辺 / windowDivisor(minWindow で下限を切る。常に奇数化) */
    windowDivisor: 16,
    minWindow: 15,
    /** Sauvola: T = m * (1 + k * (s / R - 1)) */
    sauvolaK: 0.2,
    sauvolaR: 128,
    /** Bradley-Roth: T = m * (1 - k) */
    bradleyK: 0.15,
    /**
     * 「文字の濃さ」の 5 段階に対応する k。**薄い → 濃い の順**。
     *
     * どちらの方式でも k を上げるほどしきい値が下がり、結果は薄くなる(実測)。
     * 高コントラストの原稿では効きが弱く、薄い印刷や色紙ほど差が出る。
     * 既定は中央(index 2)で、従来の値と同じ。
     */
    strengthSteps: {
      sauvola: [0.34, 0.27, 0.20, 0.14, 0.08],
      bradley: [0.28, 0.21, 0.15, 0.10, 0.05],
    },
    defaultStrength: 2,
  },

  sharpness: {
    /** 元の解像度のまま見るパッチの一辺(px)。縮小するとブレが見えなくなる */
    patchSize: 512,
    /** これ未満の標準偏差のパッチは、鮮明かどうか判定できないものとして捨てる */
    minContrast: 12,
    /**
     * スコアがこれ未満なら「ブレている」。**実写での再調整が必要な暫定値(U-13)。**
     *
     * 合成画像での実測: 鮮明 0.67〜1.44 / ガウシアン 1 回 0.17〜0.48。
     * ただし合成の「鮮明」は完全な階段エッジで、実際のレンズより鋭い。
     * 実写の良好な写真はこれより低く出るはずなので、誤警告を避けて低めに置く。
     */
    blurryBelow: 0.28,
    /** これ未満なら「やや甘い」 */
    softBelow: 0.42,
  },

  output: {
    /** 既定の出力 DPI。U-03 として未確定(M0.5 の解像度実測後に決める) */
    defaultDpi: 200,
    dpiPresets: [150, 200, 300] as const,
  },

  paper: {
    /** A4。用紙サイズは mm で持ち、DPI と組み合わせて px を導出する */
    a4: { widthMm: 210, heightMm: 297 },
  },

  limits: {
    /**
     * Sauvola は二乗和の積分画像に Float64Array を要するため
     * width*height*8 バイトを追加で確保する。これを超える入力では
     * bradley にフォールバックする(端末メモリ保護)。
     */
    sauvolaMaxPixels: 8_000_000,
  },
} as const;

export const MM_PER_INCH = 25.4;

/** 用紙サイズ(mm)と DPI から出力ピクセル数を求める */
export function paperPixels(widthMm: number, heightMm: number, dpi: number) {
  return {
    width: Math.round((widthMm / MM_PER_INCH) * dpi),
    height: Math.round((heightMm / MM_PER_INCH) * dpi),
  };
}
